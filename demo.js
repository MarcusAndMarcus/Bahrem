...
(function () {
  ...
  let clienteAba = 'conta', sacola = [], conversa = [], pensando = false, sample = null;
  ...
  function telaCliente(numero) {
    ...
    if (!m) { $('#cliente').innerHTML = '<p class="vazio" style="padding:40px;text-align:center">Nenhuma mesa aberta.</p>'; return; }
    telaCliente.mesa = m;
    ...
    $('#cliente').innerHTML = `<div class="conta">
      <div class="talao">
      ...
      <div class="abinhas" id="cAbas">
        ...
        <button data-c="ajuda" class="${clienteAba === 'ajuda' ? 'aqui' : ''}">Perguntar</button>
      </div>
    ...
    if (clienteAba === 'ajuda') cliAjuda(m, c);
  }
  ...
  /* ---------- assistente: roda na conta do visitante, via sample ---------- */
  function cliAjuda(m, c) {
    $('#cPainel').innerHTML = `<div class="talao">
      <h1>Perguntar sobre o cardápio</h1>
      <p class="sub">${sample ? 'Responde sobre o cardápio e sobre esta conta. Ele sugere; quem pede é você.'
        : 'Indisponível nesta visualização — no sistema real quem chama a API é o servidor da casa, com a chave dela.'}</p>
      <div class="conversa" id="conversa">${conversa.map(falaHtml).join('') ||
        '<div class="fala ela">Pergunte o que quiser sobre o cardápio ou sobre a conta desta mesa.</div>'}</div>
      ${sample ? `<div class="perguntar">
        <div class="campo" style="flex:1;margin:0"><input id="pergunta" maxlength="300"
          placeholder="o que pedir para quatro pessoas?" ${pensando ? 'disabled' : ''}></div>
        <button class="btn forte" id="mandarP" ${pensando ? 'disabled' : ''}>Perguntar</button></div>` : ''}
      <p class="aviso-ia">No protótipo a pergunta vai pela sua própria conta Claude, e o navegador
        pede sua permissão na primeira vez. No sistema instalado, quem fala com a API é o servidor:
        a chave é da casa, há teto de mensagens por mesa e por hora, e o cardápio real vai junto
        no molde para o modelo não inventar preço.</p></div>`;
    if (!sample) return;
    $('#mandarP').onclick = () => perguntar(m, c);
    $('#pergunta').onkeydown = e => { if (e.key === 'Enter') perguntar(m, c); };
    ligarSugestoes(m);
  ...
  function ligarSugestoes(m) {
    $$('#cPainel [data-sug]').forEach(b => b.onclick = () => {
...
{"resposta":"texto","sugestoes":[ids do cardápio]}`;
  ...
  async function perguntar(m, c) {
    const texto = ($('#pergunta').value || '').trim();
    if (!texto || pensando) return;
    conversa.push({ papel: 'cliente', texto });
    pensando = true; cliAjuda(m, c);
    const menu = CARDAPIO.map(i => `${i.id}. ${i.nome} — ${real(i.preco_cent)} (${i.categoria})`).join('\n');
    const conta = m.itens.filter(i => i.estado !== 'sugerido')
      .map(i => `${i.qtd}x ${i.nome} — ${real(i.qtd * i.preco_cent)}`).join('\n') || '(nada ainda)';
    const turnos = [{ role: 'user', content:
      `${MOLDE}\n\nCARDÁPIO:\n${menu}\n\nCONTA DA MESA ${m.numero}:\n${conta}\ntotal ${real(c.total_cent)} · ${m.pessoas} pessoa(s)` },
      ...conversa.slice(-7).map(f => ({ role: f.papel === 'cliente' ? 'user' : 'assistant', content: f.texto }))];
    try {
      ...
      conversa.push({ papel: 'assistente', texto: (j?.resposta || text || '').trim(),
        sugestoes: (j?.sugestoes || []).map(Number).filter(i => ids.has(i)).slice(0, 3) });
    } catch (e) {
      conversa.push({ papel: 'assistente', texto: e?.code === 'not_granted'
        ? 'você não autorizou a chamada — no sistema real quem paga e autoriza é a casa'
        : `não deu para responder agora (${esc(e?.code || e?.message || 'erro')})` });
    } finally { pensando = false; cliAjuda(m, c); }
  }
  ...
  let conversaEquipe = [], pensandoEquipe = false;
...
Responda SEMPRE um único objeto JSON: {"resposta":"texto","mesas":[números de mesa]}`;
  ...
  function telaIA() {
    ...
    $('#gaveta').setAttribute('aria-hidden', 'false');
    $('#gCorpo').innerHTML = `
      <p class="nota" style="margin-top:0">${sample
        ? 'Ele enxerga o retrato do salão neste instante. Responde e aponta; quem age é você.'
        : 'Indisponível nesta visualização — no sistema real quem chama a API é o servidor da casa.'}</p>
      <div class="conversa" id="convIA">${conversaEquipe.map(f =>
        `<div class="fala ${f.papel === 'eu' ? 'eu' : 'ela'}">${esc(f.texto)}
          ${(f.mesas || []).length ? `<div class="mesas-apontadas">${f.mesas.map(n =>
            `<button class="btn miudo" data-ir="${n}">mesa ${n}</button>`).join('')}</div>` : ''}</div>`).join('') ||
        `<div class="fala ela">Por exemplo: qual mesa está esperando comida há mais tempo?
          Quem pediu a conta? Quanto tem em aberto?</div>`}</div>
      ${sample ? `<div class="perguntar"><div class="campo"><input id="pIA" maxlength="300"
        placeholder="o que está pegando agora?" ${pensandoEquipe ? 'disabled' : ''}></div>
        <button class="btn forte" id="mIA" ${pensandoEquipe ? 'disabled' : ''}>Perguntar</button></div>` : ''}
      <p class="aviso-ia">No protótipo a pergunta vai pela sua conta Claude. No sistema instalado,
        quem fala com a API é o servidor, com a chave da casa e teto de chamadas por hora.</p>`;
    $('#gPe').innerHTML = '';
    $$('#gCorpo [data-ir]').forEach(b => b.onclick = () => abrirGaveta(Number(b.dataset.ir)));
    if (!sample) return;
    $('#mIA').onclick = perguntarEquipe;
    $('#pIA').onkeydown = e => { if (e.key === 'Enter') perguntarEquipe(); };
    $('#convIA').scrollTop = 1e6;
  }

  async function perguntarEquipe() {
    const texto = ($('#pIA').value || '').trim();
    if (!texto || pensandoEquipe) return;
    conversaEquipe.push({ papel: 'eu', texto });
    pensandoEquipe = true; telaIA();
    const turnos = [{ role: 'user', content: `${MOLDE_EQUIPE}\n\nESTADO DO SALÃO:\n${retrato()}` },
      ...conversaEquipe.slice(-7).map(f => ({ role: f.papel === 'eu' ? 'user' : 'assistant', content: f.texto }))];
    try {
      const { text } = await sample(turnos, { modelTier: 'quick', cache: false });
      let j = null;
      try { j = JSON.parse(String(text).replace(/^```(?:json)?/i, '').replace(/```$/, '').trim()); } catch {}
      const existem = new Set(mesas.filter(m => m.pessoas).map(m => m.numero));
      conversaEquipe.push({ papel: 'ela', texto: (j?.resposta || text || '').trim(),
        mesas: (j?.mesas || []).map(Number).filter(n => existem.has(n)).slice(0, 5) });
    } catch (e) {
      conversaEquipe.push({ papel: 'ela', texto: e?.code === 'not_granted'
        ? 'você não autorizou a chamada — no sistema real quem autoriza é a casa'
        : `não deu para responder agora (${e?.code || e?.message || 'erro'})` });
    } finally { pensandoEquipe = false; telaIA(); }
  }

  $('#abrirIA').onclick = e => { e.preventDefault(); telaIA(); };
  ...
  if (window.claude?.use) {
    claude.use('sample').then(s => {
      sample = s;
      if (aba === 'cliente' && clienteAba === 'ajuda') telaCliente(telaCliente.mesa?.numero);
    }).catch(() => {});
  }
})();
