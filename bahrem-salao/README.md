...
A paleta é minha leitura da casa (noite, madeira, latão, chopp) — **não é
amostra do manual de marca do Bahrem**. Se vocês tiverem os hexes oficiais,
trocar o bloco `:root` no topo de `public/bahrem.css` muda o sistema inteiro:

```css
--garrafa: #101512;  --tampo: #17201b;  --risco: #2c3a30;
--papel:   #f2e9d8;  --latao: #c08a2e;  --chopp: #f0b429;
--brasa:   #a8342c;  --folha: #6f7a63;
```

Tipografia: Archivo para número de mesa e interface, IBM Plex Mono para comanda e
telemetria. Nenhuma foto da casa foi embutida — as fotos que circulam do Bahrem
são de terceiros (Google Maps, imprensa) e não dá para usar sem licença.
